...
class QxubConfig:
    ...
    def get_queue_preferences(self) -> Dict[str, Any]:
        ...
        return self.get("queue_preferences", {
    ...
    def get_default_resources(self) -> Dict[str, Any]:
        ...
        return self.get("default_resources", {
            "cpus": 1,
            "memory": "4GB",
            "walltime": "1:00:00"
        })
...
def validate_config() -> List[str]:
    ...
    if queue_prefs.get("optimization") not in valid_optimizations:
        issues.append(f"Invalid optimization preference: {queue_prefs.get('optimization')}")
    
    if queue_prefs.get("adjustment_policy") not in valid_policies:
        issues.append(f"Invalid adjustment policy: {queue_prefs.get('adjustment_policy')}")
