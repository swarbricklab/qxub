...
run_edge_case_test() {
    ...
    local dry_cmd=$(echo "$test_cmd" | sed 's/qxub /qxub exec --dry-run /')
...
test_advanced_quoting() {
    ...
    run_edge_case_test "Nested quotes - single inside double" \
        "qxub exec --env base -- echo \"outer 'single' quotes\""

    run_edge_case_test "Nested quotes - double inside single" \
        "qxub exec --env base -- echo 'outer \"double\" quotes'"

    # Python with complex quotes
    run_edge_case_test "Python complex nested quotes" \
        "qxub exec --env base -- python -c \"print('single inside double: \\\"nested double\\\"')\""
    ...
    run_edge_case_test "Mixed quotes in bash command" \
        "qxub exec --env base -- bash -c 'echo \"level1 '\\''level2'\\'' back to level1\"'"
}
...
test_variable_expansion() {
    ...
    run_edge_case_test "Submission-time variable expansion" \
        "qxub exec --env base -- echo \"Submitted by: \$USER\"" \
        0 "$USER"
    ...
    run_edge_case_test "Execution-time variable expansion (escaped)" \
        "qxub exec --env base -- echo \"Job user: \\\$USER\""

    # PBS-specific variables (only available at execution)
    run_edge_case_test "PBS job variables" \
        "qxub exec --env base -- echo \"PBS job ID: \\\$PBS_JOBID\""

    # Mixed expansion scenario
    run_edge_case_test "Mixed expansion timing" \
        "qxub exec --env base -- echo \"Job \\\$PBS_JOBID submitted by \$USER on \\\$HOSTNAME\""

    # Parameter expansion
    run_edge_case_test "Parameter expansion" \
        "qxub exec --env base -- bash -c \"VAR=test; echo \\\"\\\${VAR:-default}\\\"\""
}
...
test_shell_metacharacters() {
    ...
    run_edge_case_test "Command substitution with \$()" \
        "qxub exec --env base -- bash -c \"echo \\\"Today is \\\$(date)\\\"\""
    ...
    run_edge_case_test "Nested command substitution" \
        "qxub exec --env base -- bash -c \"echo \\\"\\\$(basename \\\$(dirname \\\$(pwd)))\\\"\""

    # Here documents
    run_edge_case_test "Here document simulation" \
        "qxub exec --env base -- bash -c \"cat <<EOF > output.txt
Line 1
Line 2 with \\\$VAR
EOF\""
    ...
    run_edge_case_test "Process substitution" \
        "qxub exec --env base -- bash -c \"echo \\\"Compare: \\\$(diff <(echo a) <(echo b) || true)\\\"\""

    # Globbing patterns
    run_edge_case_test "Glob patterns" \
        "qxub exec --env base -- bash -c \"echo file*.txt\""

    run_edge_case_test "Brace expansion" \
        "qxub exec --env base -- bash -c \"echo file{1,2,3}.txt\""
}
...
test_unicode_encoding() {
    ...
    run_edge_case_test "Unicode characters" \
        "qxub exec --env base -- echo \"Unicode: 你好世界\""

    # Emoji
    run_edge_case_test "Emoji characters" \
        "qxub exec --env base -- echo \"Emoji: 🚀🔬📊\""

    # Accented characters
    run_edge_case_test "Accented characters" \
        "qxub exec --env base -- echo \"Accents: café résumé naïve\""

    # Unicode in Python
    run_edge_case_test "Unicode in Python" \
        "qxub exec --env base -- python -c \"print('Greek: αβγδε')\""

    # Control characters
    run_edge_case_test "Control characters" \
        "qxub exec --env base -- printf \"Tab:\\\\t Newline:\\\\n\""

    # ANSI escape sequences
    run_edge_case_test "ANSI color codes" \
        "qxub exec --env base -- echo -e \"\\\\x1b[31mRed text\\\\x1b[0m\""
}
...
test_large_commands() {
    ...
    run_edge_case_test "Large command payload" \
        "qxub exec --env base -- $LONG_CMD"

    # Complex pipeline
    run_edge_case_test "Complex pipeline" \
        "qxub exec --env base -- bash -c \"echo 'test data' | tee /tmp/qxub_test_out | wc -l | tee -a /tmp/qxub_test_log\""

    # Multiple commands with error handling
    run_edge_case_test "Command chain with error handling" \
        "qxub exec --env base -- bash -c \"echo 'step1' && echo 'step2' && echo 'step3' || echo 'error'\""

    # Loop with variables
    run_edge_case_test "Loop with variable expansion" \
        "qxub exec --env base -- bash -c \"for i in {1..3}; do echo \\\"Iteration \\\$i: \\\$(date)\\\"; done\""
}
...
test_pre_post_interactions() {
    ...
    run_edge_case_test "Pre/post variable sharing" \
        "qxub exec --env base --pre 'export MYVAR=pre_value' --post 'echo Final: \\\$MYVAR' -- echo 'Main: \\\$MYVAR'"

    # Complex pre command with quotes
    run_edge_case_test "Complex pre command" \
        "qxub exec --env base --pre 'echo \\\"Pre: \\\$(date)\\\"' -- echo 'Main command'"

    # Post command with pipeline
    run_edge_case_test "Complex post command" \
        "qxub exec --env base --post 'echo \\\"Done\\\" | tee -a /tmp/qxub_completion_log' -- echo 'Main work'"
}
