...
def test_config_creation():
    ...
    if (default_resources.get("cpus") == 1 and 
        queue_prefs.get("optimization") == "balanced"):
...
def test_config_file_loading():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            ...
            # Test loaded values
            if (config.get_default_platform() == "test_platform" and
                config.get("verbosity") == 2 and
                config.get_default_resources()["cpus"] == 2):
...
def test_logging_setup():
    ...
    for verbosity in [0, 1, 2, 3]:
        ...
        if logger.level == expected_levels[verbosity]:
            ...
            print(f"  ❌ Verbosity {verbosity} -> {logging.getLevelName(logger.level)}, expected {logging.getLevelName(expected_levels[verbosity])}")
            return False
