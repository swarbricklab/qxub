...
from qxub.platform import (
    QueueLimits, WalltimeRule, Queue, Platform, PlatformLoader,
    QueueSelector, select_best_queue, ResourceValidationResult
)
        ...
        "queues": [
            {
                ...
                "walltime_rules": [
                    {"cores": "1-48", "max_walltime": "48:00:00"}
                ],
            ...
            },
            {
                ...
                "walltime_rules": [
                    {"cores": "1-12", "max_walltime": "2:00:00"}
                ]
            },
            {
                "name": "gpu",
                ...
                "walltime_rules": [
                    {"cores": "1-12", "max_walltime": "24:00:00"}
                ]
            }
        ],
...
def test_queue_validation():
    ...
    test_cases = [
        ...
        ({"cpus": 4, "memory": "256GB", "walltime": "2:00:00"}, False),  # Too much memory
        ({"cpus": 4, "memory": "8GB", "walltime": "72:00:00"}, False),  # Too much walltime
    ]
    ...
    for resources, should_be_valid in test_cases:
        ...
        if result.is_valid == should_be_valid:
            ...
            print(f"  ❌ {resources} -> valid={result.is_valid}, expected {should_be_valid}")
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for resources, expected_queue in test_cases:
            ...
            if result.best_queue == expected_queue:
                ...
                print(f"  ❌ {resources} -> {result.best_queue}, expected {expected_queue}")
                if result.warnings:
...
def test_cost_estimation():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        if result.estimated_cost and abs(result.estimated_cost - expected_cost) < 0.001:
            ...
            print(f"  ❌ Cost estimation: {result.estimated_cost}, expected {expected_cost}")
            return False
